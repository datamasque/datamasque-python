...
from tests.helpers import (
    ...
    snowflake_connection_config_spcs,
)
...
def test_list_connnections(client):
    with requests_mock.Mocker() as m:
        ...
        assert snowflake_connection.snowflake_stage_location is None
        ...
        assert minimal_snowflake_connection.snowflake_stage_location is None
...
def test_snowflake_connection_spcs_round_trips():
    """`spcs` survives config → request body → server response → config."""
    config = snowflake_connection_config_spcs()
    api_dict = config.model_dump(exclude_none=True, by_alias=True, mode="json")
    assert api_dict["snowflake_stage_location"] == "spcs"

    listed = SnowflakeConnectionConfig.model_validate(
        {**api_dict, "id": "spcs-connection-id", "mask_type": "database", "password_encrypted": "encrypted"}
    )
    assert listed.snowflake_stage_location is SnowflakeStageLocation.spcs
    assert listed.model_dump(exclude_none=True, by_alias=True, mode="json")["snowflake_stage_location"] == "spcs"
...
@pytest.mark.parametrize("stage_location", ["aws_s3", "azure_blob_storage", "local"])
def test_snowflake_connection_model_validate_drops_external_stage_location(stage_location):
    """Stage locations from servers older than 3.26.18 parse as ``None``."""
    payload = {
        "id": "f0557fb3-1c9a-4cb1-bcf4-9699cf496bf7",
        "name": "snowflake",
        "mask_type": "database",
