...
class RunClient(BaseClient):
    ...
    def get_db_discovery_result_report(self, run_id: RunId, include_selection_column: bool = True) -> Union[str, bytes]:
        ...
        Returns CSV text (`str`), or a zip of numbered CSV parts as `bytes` when the report is
        large enough that the server splits it.
        ...
        if response.headers.get("X-DM-Download-Format") == "zip":
